...
def main(config):
    ...
    data_dir = './spmel'
    # spectrogram_dir = os.path.join(data_dir, 'p225') 
    # Data loader.
    vcc_loader = get_loader(config.data_dir, config.batch_size, config.len_crop)
    # vcc_loader = get_loader(config.spectrogram_dir, config.batch_size, config.len_crop)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--len_crop', type=int, default=256, help='dataloader output sequence length') # was 128
